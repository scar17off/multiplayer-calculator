...
function App() {
  ...
  const drawPlayers = (players) => {
    ...
    players.forEach(plr => {
      ...
      // Draw cursor image at its natural size
  ...
  // Add this new useEffect to log when calculatorType changes
  useEffect(() => {
    console.log('Calculator type state changed to:', calculatorType);
  }, [calculatorType]);

  // Use useCallback to memoize the renderCalculator function
